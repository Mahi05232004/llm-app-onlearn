"""DSA Tutor Agent factory — DeepAgent with skills-from-disk architecture.
...
from app.tutor.dsa.prompts.system_prompts import BASE_SYSTEM_PROMPT
...
# (fixes stale-state bug where checkpointed empty skills_metadata prevents loading)
...
def _make_backend(runtime):
    ...
    This allows SkillsMiddleware to discover skills from disk while the agent's
    memory files (AGENTS.md, short_term_plan.md) live in MongoDB via StoreBackend.
    """
...
def create_dsa_agent(
    *,
    ...
    """Create the DSA tutor DeepAgent."""

    # Semantic summarization: offloads old conversation chunks to vector store
