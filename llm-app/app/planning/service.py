...
from app.planning.planner_agent import generate_topic_ordering
...
class PlanningService:
    ...
    async def generate_initial_plan(
        ...
        # Step 5: Record trace for eval dashboard
        try:
            from tests.tracing.tracer import turn_tracer
            if not user_id:
                logger.error("generate_initial_plan MUST be called with a user_id for tracing.")
                raise ValueError("user_id is required for plan tracking")
            plan_session_id = f"plan_{course_id}_{user_id}"

            # Serialize profile for store_before (convert datetime to string)
            safe_profile = {}
            for k, v in profile_dict.items():
                if hasattr(v, 'isoformat'):
                    safe_profile[k] = v.isoformat()
                else:
                    safe_profile[k] = v

            focus_area_labels = [
                fa.get("label", str(fa)) if isinstance(fa, dict) else str(fa) 
            ...
            turn_tracer.record_turn(
                ...
                user_id=user_id,
                input_message=f"Generate learning plan for: {student_profile.goal}",
                input_files={},
                output_response=(
                    f"Generated {plan.total_topics}-topic, {plan.total_weeks}-week plan. "
                    f"Focus areas: {', '.join(focus_area_labels)}. "
                    f"LLM reasoning: {planner_result.get('reasoning', 'N/A')}"
                ),
                output_files={},
                agent_name="plan_generator",
                tools_called=["generate_topic_ordering", "build_weekly_plan"],
                mode="plan_generation",
                module=course_id,
                agent_type="plan_generator",
                thread_id=f"plan_{course_id}",
                store_before={
                    "profile": str(safe_profile),
                    "available_weeks": str(available_weeks),
                    "total_budget_minutes": str(total_budget_minutes),
                    "weekly_hours": str(student_profile.weekly_hours),
                    "feedback": str(feedback or ""),
                },
                store_after={
                    "plan_topics": str(plan.total_topics),
                    "plan_weeks": str(plan.total_weeks),
                    "plan_version": str(getattr(plan, 'plan_version', 1)),
                    "focus_areas": str(focus_areas),
                    "reasoning": str(planner_result.get("reasoning", "")),
                },
            )
            logger.info(f"Plan generation trace recorded: {plan_session_id}")
        except Exception as e:
            logger.warning(f"Failed to record plan trace (non-blocking): {e}")

        return plan, progress

    async def stream_initial_plan(
        self,
        student_profile: StudentProfile,
        course_id: str = "dsa",
        started_at: Optional[datetime] = None,
        ...
        """
        Stream the initial plan generation via an async generator.
        Yields string chunks during LLM thought process, then yields the final fully-built LearningPlan object.
        """
        from app.planning.planner_agent import stream_topic_ordering
        if started_at is None:
            started_at = datetime.now(UTC)

        profile_dict = student_profile.model_dump()
        logger.info("Generating topic ordering via stream planner agent...")
        ...
        if days_until_target <= 0:
            ...
            import re
            timeline_raw = student_profile.timeline or profile_dict.get("timeline", "")
            for field in [timeline_raw, student_profile.goal, str(student_profile.target_date)]:
                if weeks_match := re.search(r'(\d+)\s*weeks?', str(field), re.IGNORECASE):
                    estimated_weeks = int(weeks_match.group(1))
                    break
                if months_match := re.search(r'(\d+)\s*months?', str(field), re.IGNORECASE):
                    estimated_weeks = int(months_match.group(1)) * 4
                    break
            available_weeks = max(estimated_weeks, 4)
        else:
        ...
        # Async generator from planner agent
        planner_result_dict = None
        async for chunk in stream_topic_ordering(
            profile_dict,
            module_id=course_id,
            feedback=feedback,
            available_weeks=available_weeks,
            ...
            if isinstance(chunk, str):
                ...
                planner_result_dict = chunk

        if not planner_result_dict:
            raise ValueError("Planner agent completed without yielding a final result dictionary")

        ordered_topic_ids = planner_result_dict["ordered_topics"]
        focus_areas = planner_result_dict["focus_areas"]

        all_questions = get_questions(course_id=course_id)
        questions_by_id = {q["question_id"]: q for q in all_questions}
        ordered_questions = [questions_by_id[qid] for qid in ordered_topic_ids if qid in questions_by_id]
        ...
        yield {
            "plan": plan,
            "progress": progress,
            "planner_result_dict": planner_result_dict,
            "available_weeks": available_weeks,
            "total_budget_minutes": total_budget_minutes,
        }
