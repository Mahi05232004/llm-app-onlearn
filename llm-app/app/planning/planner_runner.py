...
from app.tutor.config import get_tutor_model
from app.planning.plan_store import plan_store
from app.planning.planner_prompt import PLANNER_AGENT_PROMPT
from app.planning.planner_tools import create_planner_tools
...
async def run_planner_agent(
    ...
    Args:
        ...
        module: Which module's plan to update ('dsa' or 'ds')

    Returns:
        Summary string of what was updated, suitable for the Master
        Agent to reference in its response to the student.
    """
    # 1. Read current plan data from MongoDB (module-aware)
    raw_data = plan_store.get_plan_data(user_id, module=module)
    ...
    # 4. Create the planner agent (pro model for best quality plans)
    model = get_tutor_model(model_type="pro")
    planner_agent = create_deep_agent(model=model, system_prompt=PLANNER_AGENT_PROMPT, tools=tools)
    ...
    # 6. Write updated plan data back to MongoDB (module-aware)
